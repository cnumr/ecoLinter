...
module.exports = {
  ...
  create: function(context) {
    ...
    function noCallWithStringArg(node) {
      let method = node.callee.type === 'MemberExpression' ? node.callee.property.name : node.callee.name;

      if ((method === 'setTimeout' || method === 'setInterval') && typeof node.arguments[0].value === 'string') {
        context.report({
          node,
